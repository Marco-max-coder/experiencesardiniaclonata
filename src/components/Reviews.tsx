...
type Review = {
  ...
  rating: number;   // 1..5
  comment: string;
  date?: string;    // "YYYY-MM-DD" opzionale
};
...
export default function Reviews() {
  ...
  useEffect(() => {
    // cache-busting per vedere subito gli aggiornamenti dopo il commit
    fetch(`/data/reviews.json?v=${Date.now()}`)
      ...
      .then((data: Review[]) => {
        // Ordina (opzionale): dalla più recente
  ...
  if (error) {
    return <p>Le recensioni non sono al momento disponibili.</p>;
  }
  if (!reviews) {
    return <p>Caricamento recensioni…</p>;
  }
  if (reviews.length === 0) {
    return <p>Nessuna recensione pubblicata al momento.</p>;
  }
